...
public class RerouteUtil {
    ...
    public void reroute(Vehicle vehicle) {
        ...
        Optional<List<DriveOrder>> optOrders = tryReroute(unfinishedOrders, vehicle, rerouteSource);
        ...
        if (optOrders.isPresent()) {
            ...
            LOG.debug("Found a new route for {} from point {}: {}", vehicle.getName(), rerouteSource.getName(), newDriveOrders);
        } else {
            ...
            unfinishedOrders = markRestrictedSteps(unfinishedOrders, new ExecutionTest(configuration.reroutingImpossibleStrategy(), rerouteSource));
    ...
    private void adjustFirstDriveOrder(List<DriveOrder> newDriveOrders, Vehicle vehicle, TransportOrder originalOrder, Point rerouteSource) {
        ...
        if (vehicle.hasProcState(Vehicle.ProcState.PROCESSING_ORDER)) {
            if (isPointDestinationOfOrder(rerouteSource, originalOrder.getCurrentDriveOrder())) {
                ...
                DriveOrder newCurrentOrder = mergeDriveOrders(originalOrder.getCurrentDriveOrder(), newDriveOrders.get(0), vehicle);
    ...
    public DriveOrder mergeDriveOrders(DriveOrder orderA, DriveOrder orderB, Vehicle vehicle) {
        ...
        DriveOrder mergedOrder = new DriveOrder(orderA.getDestination()).withState(orderA.getState()).withTransportOrder(orderA.getTransportOrder()).withRoute(mergedRoute);
     ...
     */
    public Optional<List<DriveOrder>> tryReroute(List<DriveOrder> driveOrders, Vehicle vehicle, Point sourcePoint) {
        LOG.debug("Trying to reroute drive orders for {} from {}: {}", vehicle.getName(), sourcePoint, driveOrders);
        Optional<List<DriveOrder>> optDriveOrders = router.getRoute(vehicle, sourcePoint, new TransportOrder("reroute-dummy", driveOrders));
    ...
    public List<Step> getFutureSteps(Vehicle vehicle) {
        ...
        if (!controller.getCommandsSent().isEmpty()) {
            ...
            currentSteps = currentSteps.subList(currentSteps.indexOf(lastCommandSent.getStep()) + 1, currentSteps.size());
        }
    ...
    public boolean routesEquals(List<DriveOrder> ordersA, List<DriveOrder> ordersB) {
        List<Route> routesA = ordersA.stream().map(order -> order.getRoute()).collect(Collectors.toList());
        List<Route> routesB = ordersB.stream().map(order -> order.getRoute()).collect(Collectors.toList());
    ...
    private Step findStepWithSource(Point sourcePoint, List<Step> steps) {
        ...
        return steps.stream().filter(step -> Objects.equals(step.getSourcePoint(), sourcePoint)).findFirst().get();
    }
    ...
    private List<Step> updateRouteIndices(List<Step> steps) {
        ...
        for (int i = 0; i < steps.size(); i++) {
            ...
            updatedSteps.add(new Step(currStep.getPath(), currStep.getSourcePoint(), currStep.getDestinationPoint(), currStep.getVehicleOrientation(), i, currStep.isExecutionAllowed()));
        }
        return updatedSteps;
    }

    private List<Path> stepsToPaths(List<Step> steps) {
        return steps.stream().map(step -> step.getPath()).collect(Collectors.toList());
    }
